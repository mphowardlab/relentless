...
class RelativeEntropy(ObjectiveFunction):
    ...
    quantifies the statistical distance (degree of overlap) of two probability
    distributions. A known target statistical mechanical ensemble is defined, and
    has a distribution :math:`p_0`. Additionally, a model ensemble parametrized
    by a given set of pair potentials, is generated by a simulation, and has a
    distribution :math:`p`. Then, the relative entropy from the model distribution
    to the target distribution is:
    ...
    completely, and positive otherwise. Hence, minimization of :math:`S_{\rm rel}`
    can be used to parametrize a model that reproduces the target ensemble.

    The value of the relative entropy is not readily determined from  molecular
    simulations, so this :class:`ObjectiveFunction` does not return a value.
    However, the relative entropy gradient with respect to design variables :math:`\mathbf{x}`
    is much easier to compute. Currently, the :class:`RelativeEntropy` objective
    function supports :class:`~relentless.potential.pair.PairPotential` interactions.
    These interactions are characterized by :math:`g_{ij}(r)`, an :class:`~relentless.ensemble.RDF`
    for each pair of interacting types :math:`(i,j)` in each :class:`~relentless.ensemble.Ensemble`.
    ...
    :math:`u_{ij}(r)` is the pair potential in the *model* ensemble. The corresponding
    ensemble properties and rdf of the *target* ensemble are denoted with
    subscript :math:`0`.
    ...
    certain hyperparameters are system-dependent. To circumvent this, the default
    setting is to normalize :math:`S_{\rm rel}=S_{\rm rel}/V_0` using ``extensive=False``.
    The extensive relative entropy can still be computed by setting ``extensive=True``.
