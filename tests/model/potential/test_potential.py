...
class MockPotential(relentless.model.potential.Potential):
    ...
    def __init__(self, types, params, container=None, name=None):
        super().__init__(types, params, container, name)
...
class MockOneParameterPotential(MockPotential):
    def __init__(self, types, name=None):
        super().__init__(types, params=("m",), name=name)
...
class test_Potential(unittest.TestCase):
    ...
    def test_default_name(self):
        p = MockOneParameterPotential(types=("1",))
        p.coeff["1"]["m"] = 2.0
        self.assertEqual(p.name, f"__u[{p.id}]")

        # save to temporary file
        temp = tempfile.NamedTemporaryFile()
        p.save(temp.name)

        # default - should discard because it was an autogenerated name
        p2 = MockOneParameterPotential.from_file(temp.name)
        self.assertEqual(p2.id, p.id + 1)
        self.assertEqual(p2.name, f"__u[{p2.id}]")

        # override
        p2 = MockOneParameterPotential.from_file(
            temp.name, name="override_default_name"
        )
        self.assertEqual(p2.id, p.id + 2)
        self.assertEqual(p2.name, "override_default_name")

        # discard
        p2 = MockOneParameterPotential.from_file(temp.name, name=False)
        self.assertEqual(p2.id, p.id + 3)
        self.assertEqual(p2.name, f"__u[{p2.id}]")

        # keep - should error because name is taken
        with self.assertRaises(ValueError):
            p2 = MockOneParameterPotential.from_file(temp.name, name=True)

    def test_nondefault_name(self):
        p = MockOneParameterPotential(types=("1",), name="mock")
        p.coeff["1"]["m"] = 2.0
        ...
        # default - should discard because name is taken
        p2 = MockOneParameterPotential.from_file(temp.name)
        self.assertEqual(p2.id, p.id + 1)
        self.assertEqual(p2.name, f"__u[{p2.id}]")

        # override
        p2 = MockOneParameterPotential.from_file(temp.name, name="mock2")
        self.assertEqual(p2.id, p.id + 2)
        self.assertEqual(p2.name, "mock2")
