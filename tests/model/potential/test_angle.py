...
class test_AnglePotential(unittest.TestCase):
    ...
    def test_derivative_values(self):
        ...
        # test with no cutoffs
        d = p.derivative(type_=("1"), var=x, theta=0.5)
        self.assertAlmostEqual(d, 0.5)
        d = p.derivative(type_=("1"), var=x, theta=[0.25, 0.75])
    ...
    def test_derivative_types(self):
        ...
        d = q.derivative(type_=("1"), var=z, theta=2.0)
        ...
        with self.assertRaises(TypeError):
            d = q.derivative(type_=("1"), var=a, theta=2.0)
